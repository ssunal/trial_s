...
require_once("yenClass.config.php");
 /* this program allows you to add new property to database. Now, It doeasn't work because I couldn't get any email about my question */
 if (isset($_POST['uuid']))
    {
 ...
 <form id="profile_acc"   action='<?php echo $yen_class->GetSelfScript(); ?>' method="post" style="margin-top:0px;margin-left: 50px;margin-right: 50px">
  ...
  <label style="margin-left:0px;margin-top:-25px;padding:0px" > My Details </label>
    <div class="form with-line "  style="float:left;margin-left:0px;margin-top:0px;width:100%">
             <h1>Add property</h1>
          <h5>uuid:</h5>
           <input type='text' id='c' name='uuid' value='<?php echo $json->uuid ?>'>


           <h5>property_type_id:</h5>
            <input type='text' id='cr' name='property_type_id' value='<?php echo $json->property_type_id ?>'>
